...
function onProfileClick() {
    window.location.href = '../chat-profile/index.html';
}
...
async function onSubmitRegistrationClick() {
    ...
    const repeatPassword = inpRepeatPassword.value;
...
async function checkAuth() {
    ...
    try {
        ...
        if (result.isLogin) {
            ...
            //setInterval(getMyGroups, 20000);
            //! Пусть группы загружайтся 1 раз, слишком много проблем с этим
        } else {
...
async function loadMessages(anyway = false) {
    if (openPage !== 'messages' && !anyway)
        return;

    const data = {
        type: 'getPersonalMessages',
        startFromIdMessage: messages.length > 0 ? Number(messages[messages.length - 1].id) + 1 : 1
    };
    ...
    try {
        ...
        if (result.success) {
            ...
            messagesDiv.innerHTML = '<p>Ошибка загрузки сообщений.</p>';
        }
    } catch (error) {
...
function displayMessages(messages) { //! Устарела, в будущем убрать
    const previousScrollHeight = messagesDiv.scrollHeight;
    const previousScrollTop = messagesDiv.scrollTop;
